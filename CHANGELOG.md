...
### Fixed
- **Phantom hit when equipping or removing armor that carries max health** — A locked heart bar draws current health divided by max health. Armor with a max-health bonus raises the divisor and leaves current health untouched, so the value the client is told to draw falls even though nothing hurt the player — and a falling health value is the only thing the client needs to play the hurt flash. That is why it fired in creative, where damage is impossible. Health now keeps the same share of the bar across the change, so there is no fall to react to. Set `settings.health-display-keep-ratio` to `false` to go back to armor visibly lowering your filled hearts.
- **Alchemy healing barely doing anything at high Alchemy** — The bonus was worked out from Minecraft's own heal amount, which for food regeneration is a single point, so even a maxed build added well under one heart. Healing now scales off max health instead, and applies to food as well as potions, so the reward is the same share of the bar whether the player has 100 HP or 4000. `stats.alchemy.max-health-heal-per-100-percent` moves from `0.45` to `4.0`; servers still on the old default are migrated, customised values are left alone.
- **Alchemy healing shrinking the vanilla heal** — The bonus is calculated in RPG health while the amount Minecraft passes in is already in Minecraft's own scale, but both were converted down together, so every potion and food heal returned less than it should have. Only the bonus is converted now.
- **Phantom hit and wrong hearts whenever a stat changed** — Max health was rewritten from scratch on every stat refresh, even when nothing about the player's health had changed. Each rewrite briefly dropped max health before restoring it, which the client draws as a hit, and it also erased max-health contributions from other plugins. OrbitSkills now adds only its own share as a named modifier and leaves it alone when the value has not moved, so an unchanged stat sends nothing at all.
...
- **Phantom hit when equipping or removing armor** — The client plays the hurt flash and sound whenever a health packet reports less health than it is currently drawing; it never checks whether damage actually happened, which is why the effect fired even in creative. Refreshing the heart bar was sending two health packets where one was needed, and the extra one landed mid-change as a dip. The refresh now sends a single packet, and the periodic safety net skips players whose bar is already locked instead of resending to everyone.
- **Phantom hit effect when healing, gaining health, or eating** — Same cause as the armor case above: an extra health packet reporting a momentary dip. Alchemy healing showed it most clearly, since it raises health the most at once, and eating was affected because a health packet carries food and saturation as well.
...
### Added
- **MythicMobs support** — Custom mobs can now be worth their own EXP instead of whatever their base entity is worth, so a scripted boss built on a zombie no longer pays out like a zombie. Set each mob by its internal name under `exp-sources.mob-kill.mythic` in `source/Grinding.yml`. Mobs left unlisted keep the vanilla amount.
- **`exp-sources.mob-kill.mythic-exp-percent-per-level`** — Extra EXP per MythicMobs level, so a high-level mob is worth more than a low-level one of the same kind without needing its own entry. Defaults to `0` (off).
- **MMOItems support** — MMOItems weapons are now scored by the type their author gave them rather than by material. A custom staff or dagger built on a stick used to read as an empty hand and score as an unarmed Power hit; it now counts as the weapon it is. The mapping lives in `hooks.MMOItems.weapon-stats` and covers the common types out of the box.
- **`hooks.MythicMobs.enabled` / `hooks.MMOItems.enabled`** — Turn either integration off without uninstalling anything.
- **`settings.health-display-keep-ratio`** — Keeps the heart bar still when something changes max health. Defaults to `true`. Set `false` if you would rather armor visibly lower your filled hearts.
...
### Notes
- MythicMobs and MMOItems support is optional and adds no dependency to the plugin. Each one is used only when that plugin is installed, and OrbitSkills keeps its own stats and leveling either way — no stat values are read from either plugin, only which mob or item something is.
- Paper servers are unaffected in behaviour; the refresh only re-applies the existing heart scale.
